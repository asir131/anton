...
export function Checkout() {
  ...
  const cartTotal = cart?.summary.total_price ?? 0;
  // Every 100 points = $1 discount
  const pointsDiscount = usePoints
    ? Math.min(Math.floor(availablePoints / 100), cartTotal)
    : 0;
  const finalAmount = Math.max(0, cartTotal - pointsDiscount);

  // Handle payment success
  useEffect(() => {
    if (paymentStatus?.tickets_created && step === 3) {
      // Clear cart after successful payment
      clearCart();
    }
  }, [paymentStatus, step, clearCart]);
  ...
  const handleContinueToPayment = async () => {
    ...
    try {
      const points_to_redeem = usePoints ? availablePoints : 0;
  ...
  const pollForTicketCreation = async (paymentIntentId: string) => {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (data.success && status === "succeeded" && !ticketsCreated) {
          console.log("Payment succeeded, waiting for tickets to be created...");
          // Continue polling
        }
        ...
        if (i >= 20 && status === "pending") {
          console.warn("Payment still pending after 20 attempts - webhook may not be configured");
        }
    ...
    setIsProcessing(false);
    toast.warning("Payment processing timed out. The payment may have succeeded - please check your entries page or contact support.");
      ...
      <div className="container-premium max-w-4xl">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 3 && (
            ...
            >
              {isProcessing || (!paymentStatus && paymentIntentId) || (paymentStatus && paymentStatus.status !== "succeeded" && !paymentStatus.tickets_created) ? (
                <>
                  <motion.div
                    initial={{
                      scale: 0,
                  ...
                  {paymentStatus && (
                    <div className="text-sm text-text-secondary space-y-2 mb-4 p-4 bg-gradient-end rounded-lg">
                      <p>Status: <span className="font-semibold capitalize">{paymentStatus.status}</span></p>
                      <p>Tickets Created: <span className="font-semibold">{paymentStatus.tickets_created ? 'Yes' : 'No'}</span></p>
                      {paymentStatus.status === "pending" && (
                        <div className="text-yellow-400 text-xs mt-3 space-y-1">
                          <p className="font-semibold">⚠ Payment Processing Issue Detected</p>
                          <p>The payment was confirmed but is stuck at "pending" status. This usually means:</p>
                          <ul className="list-disc list-inside mt-2 space-y-1">
                            <li>The Stripe webhook is not configured or not working</li>
                            <li>The backend webhook handler is not processing the payment</li>
                            <li>The payment may require additional authentication</li>
                          </ul>
                          <p className="mt-2">Please check the backend webhook configuration. See WEBHOOK_TROUBLESHOOTING.md for details.</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    <p className="text-xs text-text-secondary text-center">
                      If tickets aren't there, the webhook may not be configured. 
                      Contact support with Payment ID: <span className="font-mono text-xs">{paymentIntentId?.slice(0, 20)}...</span>
                    </p>
                  </div>
                </>
              ) : (
