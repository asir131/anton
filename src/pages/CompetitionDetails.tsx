...
export function CompetitionDetails() {
  ...
  if (isCompetitionLoading) {
    return <div className="container-premium py-8 text-center">Loading competition details...</div>;
  }

  if (error || !competition) {
    return <div className="container-premium py-8 text-center text-red-500">Error loading competition or competition not found.</div>;
  }
  
  const faqs = [{
    question: 'When will the winner be announced?',
    answer: 'The winner will be announced via live draw on the specified end date. All participants will be notified via email.'
  }, {
    question: 'How will I receive my prize?',
    answer: "If you win, we'll contact you within 24 hours to arrange delivery of your prize. All shipping costs are covered."
  }, {
  ...
  const progressPercentage = Math.min(Math.round(competition.tickets_sold / competition.max_tickets * 100), 100);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addToCart({ competition_id: competition._id, quantity }).unwrap();
  ...
  return <div className="py-8">
      ...
      duration: 0.5
        ...
        <SafeImage src={competition.image_url} alt={competition.title} className="w-full h-full object-cover" />
          ...
          delay: 0.3
            ...
            delay: 0.9
              ...
              <div className="flex border-b border-gray-700">
                <button onClick={() => setEntryType('online')} className={`flex-1 px-6 py-4 text-sm font-medium transition-all relative ${entryType === 'online' ? 'text-white' : 'text-text-secondary hover:text-white'}`}>
                  Online Entry
                  {entryType === 'online' && <motion.div layoutId="entryType" className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent" />}
                </button>
                <button onClick={() => {
                setEntryType('postal');
                setShowPostalModal(true);
              }} className={`flex-1 px-6 py-4 text-sm font-medium transition-all relative ${entryType === 'postal' ? 'text-white' : 'text-text-secondary hover:text-white'}`}>
                  Postal Entry
                  {entryType === 'postal' && <motion.div layoutId="entryType" className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent" />}
                </button>
              </div>
            </motion.div>
            {/* Tabs */}
            ...
            delay: 1
              ...
              <div className="flex border-b border-gray-700">
                {['details', 'faqs', 'terms'].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-4 text-sm font-medium capitalize transition-all relative ${activeTab === tab ? 'text-white' : 'text-text-secondary hover:text-white'}`}>
                    {tab === 'faqs' ? 'FAQs' : tab}
                    {activeTab === tab && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent" />}
                  </button>)}
              </div>
              <div className="p-6">
                {activeTab === 'details' && <motion.div initial={{
                opacity: 0
              }} animate={{
                opacity: 1
              }} transition={{
                duration: 0.3
              }} className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{
                __html: competition.long_description
              }} />}
                {activeTab === 'faqs' && <motion.div initial={{
                opacity: 0
              }} animate={{
                opacity: 1
              }} transition={{
                duration: 0.3
                    ...
                    {faqs.map((faq, index) => <div key={index} className="card-premium p-4">
                        <h3 className="font-semibold mb-2">{faq.question}</h3>
                        <p className="text-text-secondary">{faq.answer}</p>
                      </div>)}
                  </motion.div>}
                {activeTab === 'terms' && <motion.div initial={{
                opacity: 0
              }} animate={{
                opacity: 1
              }} transition={{
              ...
              <div className="card-premium p-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center justify-between bg-gradient-end rounded-xl p-2">
                    ...
                    <button onClick={() => setQuantity(Math.min(competition.max_per_person, quantity + 1))} className="w-10 h-10 rounded-lg bg-gradient-start hover:bg-accent transition-colors flex items-center justify-center">
                ...
                <div className="space-y-3 mb-6 p-4 bg-gradient-end rounded-xl">
                  ...
                  <div className="border-t border-gray-700 pt-3">
                    ...
                    {usePoints && <div className="flex justify-between text-accent mt-2">
                ...
                <div className="space-y-3">
                  ...
                  scale: 1.02
                } : {}} whileTap={selectedAnswer && !isAddingToCart ? {
                  scale: 0.98
                } : {}}>
      ...
      {/* Postal Entry Modal */}
      <AnimatePresence>
        {showPostalModal && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
      }} onClick={() => setShowPostalModal(false)} className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4">
            <motion.div initial={{
          opacity: 0,
          scale: 0.9,
          y: 20
          ...
          scale: 0.9,
          y: 20
        }} onClick={e => e.stopPropagation()} className="card-premium p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-center mb-6">
                <Link to="/terms-and-conditions" className="text-accent hover:underline font-medium" onClick={() => setShowPostalModal(false)}>
                  For full terms and conditions click here
                </Link>
              </div>
              <button onClick={() => setShowPostalModal(false)} className="w-full btn-premium">
                Close
              </button>
            </motion.div>
          </motion.div>}
